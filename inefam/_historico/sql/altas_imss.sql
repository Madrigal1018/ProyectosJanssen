with imss_fuente1_dateformat as (
	...
	from jia2.fct_mexico_inefam_audits_altas_imss_unops t1 
),
imss_fuente1 as (
	select 
		...
		, t1.marca
		, t2.institucion  
		, sum(cast(t1.piezas_alta_imss as decimal)) piezas
...
imss_fuente2 as (
	select 
		...
		, sum(cast(t1.importe as decimal)) importe
		, sum(cast(t1.precio as decimal)) precio
	from jia2.fct_mexico_inefam_audits_altas_imss_unops_frz t1
		left outer join (
		...
		on t1.clues = t2.clave_clues
	where
		 ref_frz_dt = (select max(ref_frz_dt) from jia2.fct_mexico_inefam_audits_altas_imss_unops_frz)
