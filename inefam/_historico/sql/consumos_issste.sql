...
issste_fuente1 as (
	select 
		...
		, sum(cast(t1.dpn as decimal)) dpn
...
issste_fuente2 as (
	select 
		...
		, sum(cast(t1.dpn as decimal)) dpn
		, sum(cast(t1.piezas_alta as decimal)) piezas
	from jia2.fct_mexico_inefam_audits_issste_consumptions_frz t1
		left outer join (
		...
		on t1.clave_clues = t2.clave_clues
	where
		 ref_frz_dt = (select max(ref_frz_dt) from jia2.fct_mexico_inefam_audits_issste_consumptions_frz)
