with normales_fuente1_dateformat as (
	select 
		*,
		case 
	...
	from jia2.fct_mexico_inefam_audits_deliveries t1 
),
normales_fuente1 as (
	select 
...
normales_fuente2 as (
	select 
		...
		, sum(cast(t1.importe as decimal)) importe
		, sum(cast(t1.precio as decimal)) precio
	from jia2.fct_mexico_inefam_audits_deliveries_frz t1
		left outer join (
		...
		on t1.clave_clues = t2.clave_clues
	where
		 ref_frz_dt = (select max(ref_frz_dt) from jia2.fct_mexico_inefam_audits_deliveries_frz)
	group by date, fuente, marca, institucion
)
select *
from normales_fuente1
union all
select *
from normales_fuente2
