...
#URL Tableau
WEB_URL = 'https://alphadogs.ai/views/03_INEFAM_FALLOS/BASEDEDATOS/2915755b-47a8-4912-bc24-f90118fee8af/30b4447e-4e3d-4405-8c7f-5b2d172b2806'
...
YEAR_BOX_SELECTOR = (By.ID, 'tableau_base_widget_LegacyCategoricalQuickFilter_3')
YEAR_BOX_ITEMS_SELECTOR = (By.ID, 'tableau_base_widget_LegacyCategoricalQuickFilter_3_menu')
ITEMS_SELECTOR = (By.CLASS_NAME, "FIText")
YEAR_BOX_APPLY_BUTTON_SELECTOR = (By.XPATH, '//*[@id="tableau_base_widget_LegacyCategoricalQuickFilter_3_menu"]/div[3]/button[2]')

DONWLOAD_BUTTON_SELECTOR = (By.ID, 'download')
DOWNLOAD_MENU_TOOLBAR_SELECTOR = (By.ID, 'viz-viewer-toolbar-download-menu')
EXPORT_CSV_BUTTON_SELECTOR = (By.CSS_SELECTOR, 'button[data-tb-test-id="export-crosstab-export-Button"]')
CSV_OPTION_SELECTOR = (By.XPATH, '/html/body/div[8]/div/div/div/div/div[2]/div/div[2]/div[2]/div/label[2]')
CROSSTAB_SHEET_321_SELECTOR = (By.CSS_SELECTOR, '#export-crosstab-options-dialog-Dialog-BodyWrapper-Dialog-Body-Id > div > div:nth-child(1) > div.f1lp596a > div > div > div:nth-child(1)')
...
WebDriverWait(web_driver, BASE_TIMEOUT_TIME) \
    .until(EC.presence_of_element_located(CROSSTAB_SHEET_321_SELECTOR))
crosstab_sheet411 = web_driver.find_element(*CROSSTAB_SHEET_321_SELECTOR)
is_selected = crosstab_sheet411.get_attribute("aria-selected")
bool_value = False if is_selected.lower() == 'false' else True
if not bool_value:
    WebDriverWait(web_driver, BASE_TIMEOUT_TIME) \
    .until(EC.element_to_be_clickable(CROSSTAB_SHEET_321_SELECTOR)) \
    .click() 

#Esperar a que aparezca el botón de descarga de csv y darle clic
WebDriverWait(web_driver, BASE_TIMEOUT_TIME)\
    .until(EC.presence_of_element_located(CSV_OPTION_SELECTOR))
WebDriverWait(web_driver, BASE_TIMEOUT_TIME)\
    .until(EC.element_to_be_clickable(CSV_OPTION_SELECTOR))\
    .click()
WebDriverWait(web_driver, BASE_TIMEOUT_TIME)\
    .until(EC.presence_of_element_located(EXPORT_CSV_BUTTON_SELECTOR))
WebDriverWait(web_driver, BASE_TIMEOUT_TIME)\
    .until(EC.element_to_be_clickable(EXPORT_CSV_BUTTON_SELECTOR))\
    .click()
sleep(180) 
...
new_file_path = os.path.join(getenv("DOWNLOAD_DIRECTORY"), "output", "scrapping_fallos", "dash_tableau.csv")
